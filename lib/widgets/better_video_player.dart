...
class BetterVideoPlayer extends StatefulWidget {
  ...
  BetterVideoPlayer({Key? key, required this.streamUrl, this.playAtMillisecondDuration}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _BetterVideoPlayerState(
        streamUrl: this.streamUrl, playAtMillisecondDuration: playAtMillisecondDuration);
  }
}
...
class _BetterVideoPlayerState extends State<BetterVideoPlayer> {
  ...
  _BetterVideoPlayerState({required this.streamUrl, this.playAtMillisecondDuration}) {
    streamUrl = streamUrl;
    ...
    _betterPlayerController.addEventsListener((event) async {
      if (event.betterPlayerEventType == BetterPlayerEventType.exception) {
        ...
        EasyDebounce.debounce('videoProgress', Duration(milliseconds: 1000), () async {
          var duration = (event.parameters?.entries
              .firstWhere((e) => e.key == 'duration', orElse: () => {'duration': null}.entries.first)
              .value as Duration?);
          if (duration != null) {
            await StorageHelper.storage.setItem('lastPlayStreamUrl', streamUrl);
            await StorageHelper.storage.setItem(
                'lastPlayDuration',
                (event.parameters?.entries.firstWhere((e) => e.key == 'progress').value as Duration)
                    .inMilliseconds);
        ...
        if (playAtMillisecondDuration != null) {
          _betterPlayerController.videoPlayerController!
              .seekTo(Duration(milliseconds: playAtMillisecondDuration!));
        }
      }
                            ...
                            child: GestureDetector(
                              ...
                              onDoubleTap: () {
                                ...
                                final skip = (videoPlayerController.position -
                                        Duration(
                                            milliseconds: _betterPlayerController.betterPlayerConfiguration
                                                .controlsConfiguration.backwardSkipTimeInMilliseconds))
                                    .inMilliseconds;
                                _betterPlayerController.seekTo(Duration(milliseconds: max(skip, beginning)));
                              },
                            ),
                          ),
                        ),
